<script>
	...
	async function removeItem(videoId) {
		...
		try {
			const res = await authFetch(`/api/playlists/${playlistId}/items/${videoId}`, { method: 'DELETE' });
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
	<div class="mb-6 flex items-start gap-4">
		<a href="/playlists" class="h-9 w-9 flex items-center justify-center border border-neutral-600 hover:border-neutral-400 transition-colors text-neutral-300 hover:text-white bg-neutral-900" aria-label="Back to playlists">
	...
	{:else if error}
		<div class="border border-neutral-800 p-8 text-center text-neutral-500 uppercase tracking-widest text-sm">
			Error: {error}
		</div>
	{:else if !playlist?.items || playlist.items.length === 0}
		<div class="border border-neutral-800 p-12 text-center flex flex-col items-center">
			...
			<a href="/" class="h-9 flex items-center px-4 text-xs font-bold uppercase tracking-widest border border-white bg-white text-black hover:bg-white/80 transition-colors">Go to Library</a>
		</div>
	{:else}
			...
			<form onsubmit={savePlaylist} class="flex flex-col gap-4">
				...
				<div class="flex justify-end gap-3 mt-4">
					<button 
						...
						onclick={() => { showEditModal = false; editName = playlist.name; editDescription = playlist.description || ''; }}
					>
						Cancel
					</button>
					<button 
