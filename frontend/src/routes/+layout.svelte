			...
			>
				<div class="p-4 flex items-center justify-between border-b border-neutral-900 bg-[#0a0a0a]">
					{#if !sidebarCollapsed}
						<a
							href="/"
							class="text-sm font-bold tracking-[0.2em] uppercase transition-colors text-white hover:text-neutral-400"
						>
							COLLECTARR
						</a>
					{/if}
				...
				<div class="flex-1 flex flex-col gap-0.5 px-2 overflow-y-auto py-2">
					{#if !sidebarCollapsed}
						<div
							class="text-[9px] font-bold tracking-[0.3em] text-neutral-600 uppercase mb-2 mt-4 px-2"
						>
							MODULES
						</div>
					{/if}
