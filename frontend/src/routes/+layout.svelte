...
{#if !$page.url.pathname.startsWith('/player') && $page.url.pathname !== '/login'}
	<div class="sm:hidden border-b border-neutral-800 bg-black text-white">
		<div class="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
			<a
				href="/"
				class="min-w-0 flex-1 truncate text-xl font-bold tracking-widest uppercase transition-colors hover:text-gray-300"
				>Collectarr</a
			>
			<button
			...
			>
				{#if $theme === 'light'}
					<svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
						<path
							d="M9 2c-1.05 0-2.05.16-3 .46 1.69 1.23 2.8 3.24 2.8 5.54 0 3.87-3.13 7-7 7-1.11 0-2.16-.26-3.09-.72C.56 16.2 3.5 19 7 19c4.97 0 9-4.03 9-9 0-4.97-4.03-9-9-9z"
						/>
					</svg>
				{:else}
					<svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
	...
	<nav
		class="navbar hidden max-w-7xl mx-auto border-b border-neutral-800 bg-black px-4 py-2 text-white sm:flex sm:px-6 lg:px-8"
	>
		<div class="flex-1 flex items-center gap-3">
			<a
				href="/"
				class="text-xl font-bold tracking-widest uppercase hover:text-gray-300 transition-colors"
				>Collectarr</a
			>
			<button
		...
		<div class="flex-none flex gap-6 items-center">
			{#if $auth.isAuthenticated}
				<span class="text-[10px] font-semibold tracking-[0.25em] uppercase text-neutral-500"
					>{$auth.username}</span
				>
			{/if}
			<a
				href="/"
				class="text-xs font-semibold tracking-widest uppercase {$page.url.pathname === '/'
					? 'text-white'
					: 'text-neutral-500'} hover:text-white transition-colors">Library</a
			>
			<a
				href="/playlists"
				class="text-xs font-semibold tracking-widest uppercase {$page.url.pathname.startsWith(
					'/playlists'
				)
					? 'text-white'
					: 'text-neutral-500'} hover:text-white transition-colors">Playlists</a
			>
			<a
				href="/torrents"
				class="text-xs font-semibold tracking-widest uppercase {$page.url.pathname === '/torrents'
					? 'text-white'
					: 'text-neutral-500'} hover:text-white transition-colors">Torrents</a
			>
			<a
				href="/settings"
				class="text-xs font-semibold tracking-widest uppercase {$page.url.pathname === '/settings'
					? 'text-white'
					: 'text-neutral-500'} hover:text-white transition-colors">Settings</a
			>
			{#if $auth.isAuthenticated}
				<button
					class="text-xs font-semibold tracking-widest uppercase text-neutral-500 hover:text-white transition-colors"
					onclick={() => logout()}
				>
					Logout
				</button>
