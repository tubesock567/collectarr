...
const qbittorrentSettingsSettingKey = "qbittorrent_settings"
...
func (s *Store) GetQBittorrentSettings() (QBittorrentSettings, error) {
	value, err := s.GetSetting(qbittorrentSettingsSettingKey)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return QBittorrentSettings{}, nil
		}
		return QBittorrentSettings{}, fmt.Errorf("get qBittorrent settings: %w", err)
	}

	settings, err := parseQBittorrentSettings(value)
	if err != nil {
...
func (s *Store) SetQBittorrentSettings(settings QBittorrentSettings) error {
	encoded, err := marshalQBittorrentSettings(settings)
	if err != nil {
		return err
	}
	if err := s.SetSetting(qbittorrentSettingsSettingKey, encoded); err != nil {
		return fmt.Errorf("save qBittorrent settings: %w", err)
	}
	return nil
}
...
func parseQBittorrentSettings(raw string) (QBittorrentSettings, error) {
	if strings.TrimSpace(raw) == "" {
		return QBittorrentSettings{}, nil
	}
	...
	if err := json.Unmarshal([]byte(raw), &settings); err != nil {
		return QBittorrentSettings{}, fmt.Errorf("decode qBittorrent settings: %w", err)
	}

	return sanitizeQBittorrentSettings(settings), nil
}

func marshalQBittorrentSettings(settings QBittorrentSettings) (string, error) {
	data, err := json.Marshal(sanitizeQBittorrentSettings(settings))
	if err != nil {
		return "", fmt.Errorf("encode qBittorrent settings: %w", err)
	}
	return string(data), nil
}
...
func sanitizeQBittorrentSettings(settings QBittorrentSettings) QBittorrentSettings {
